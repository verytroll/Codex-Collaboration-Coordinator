# Planning Archive

This directory contains historical planning documents from MVP through V7.

## Progression

- MVP: [PLAN.md](PLAN.md), [IMPLEMENTATION_TASKS.md](IMPLEMENTATION_TASKS.md), [IMPLEMENTATION_ORDER.md](IMPLEMENTATION_ORDER.md)
- V2: [PLAN_V2.md](PLAN_V2.md), [IMPLEMENTATION_TASKS_V2.md](IMPLEMENTATION_TASKS_V2.md), [IMPLEMENTATION_ORDER_V2.md](IMPLEMENTATION_ORDER_V2.md)
- V3: [PLAN_V3.md](PLAN_V3.md), [IMPLEMENTATION_TASKS_V3.md](IMPLEMENTATION_TASKS_V3.md), [IMPLEMENTATION_ORDER_V3.md](IMPLEMENTATION_ORDER_V3.md)
- V4: [PLAN_V4.md](PLAN_V4.md), [IMPLEMENTATION_TASKS_V4.md](IMPLEMENTATION_TASKS_V4.md), [IMPLEMENTATION_ORDER_V4.md](IMPLEMENTATION_ORDER_V4.md)
- V5: [PLAN_V5.md](PLAN_V5.md), [IMPLEMENTATION_TASKS_V5.md](IMPLEMENTATION_TASKS_V5.md), [IMPLEMENTATION_ORDER_V5.md](IMPLEMENTATION_ORDER_V5.md)
- V6: [PLAN_V6.md](PLAN_V6.md), [IMPLEMENTATION_TASKS_V6.md](IMPLEMENTATION_TASKS_V6.md), [IMPLEMENTATION_ORDER_V6.md](IMPLEMENTATION_ORDER_V6.md)
- V7: [PLAN_V7.md](PLAN_V7.md), [IMPLEMENTATION_TASKS_V7.md](IMPLEMENTATION_TASKS_V7.md), [IMPLEMENTATION_ORDER_V7.md](IMPLEMENTATION_ORDER_V7.md)

## When To Use The Archive

Open these files only when you need:

- historical product reasoning
- dependency tracing across phases
- release packaging tied to an older baseline
- evidence that a design or workflow predates the active phase

For current work, go back to [planning index](../INDEX.md) and start from the active phase docs.
