...
def _database_url(tmp_path: Path) -> str:
    return f"sqlite:///{(tmp_path / 'operator_dashboard.db').as_posix()}"
...
def _seed_phase(
    ...
    *,
    ...
    default_channel_key: str = "general",
    sort_order: int = 10,
    is_default: int = 0,
) -> None:
    created_at = "2026-04-01T00:00:00Z"
    ...
    asyncio.run(
        WorkContextRepository(database_url).create(
            WorkContextRecord(
                ...
                selection_reason="dashboard test",
                failure_reason=None,
                created_at=created_at,
                updated_at=created_at,
            )
...
def test_operator_dashboard_and_debug_surface(tmp_path, monkeypatch) -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            assert dashboard_payload["filters"] == {
                "session_id": None,
                "template_key": None,
                "phase_key": None,
            ...
            assert any(
                ...
                for item in dashboard_payload["bottlenecks"]
            )
            assert any(
                item["kind"] == "runtime_pool" and item["key"] == "isolated_work"
                for item in dashboard_payload["bottlenecks"]
            )
            assert any(
                item["pool_key"] == "isolated_work" and item["blocked_jobs"] == 1
                for item in dashboard_payload["runtime_pools"]
            )

            filtered_response = client.get(
                "/api/v1/operator/dashboard",
                params={"session_id": "ses_review", "phase_key": "review"},
            )
            assert filtered_response.status_code == 200
            filtered_payload = filtered_response.json()
            assert filtered_payload["filters"]["session_id"] == "ses_review"
            assert filtered_payload["filters"]["phase_key"] == "review"
            ...
            pool_response = client.get(
                "/api/v1/operator/dashboard",
                params={"runtime_pool_key": "isolated_work"},
            )
            assert pool_response.status_code == 200
            pool_payload = pool_response.json()
            assert pool_payload["filters"]["runtime_pool_key"] == "isolated_work"
            assert len(pool_payload["runtime_pools"]) == 1
            assert pool_payload["runtime_pools"][0]["pool_key"] == "isolated_work"
            assert len(pool_payload["queue_heat"]) == 1
            assert pool_payload["queue_heat"][0]["runtime_pool_key"] == "isolated_work"

            debug_response = client.get("/api/v1/operator/debug")
