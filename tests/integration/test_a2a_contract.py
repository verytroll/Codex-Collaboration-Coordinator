...
def _database_url(tmp_path: Path) -> str:
    return f"sqlite:///{(tmp_path / 'a2a_contract.db').as_posix()}"
...
def test_public_a2a_contract_surface_matches_agent_card_and_matrix(tmp_path, monkeypatch) -> None:
    ...
    try:
        with TestClient(app) as client:
            ...
            session_id = _create_session(
                client,
                title="A2A contract",
                goal="Exercise the supported public contract",
                lead_agent_id=lead_agent_id,
            )
            ...
            job = JobRecord(
                ...
                parent_job_id=None,
                title="Contract job",
                instructions="Exercise the supported public A2A surface",
                status="queued",
                hop_count=0,
                priority="normal",
            ...
            artifact = ArtifactRecord(
                ...
                content_text="Contract artifact",
                file_path=None,
                file_name="contract.txt",
                mime_type="text/plain",
                size_bytes=len("Contract artifact".encode("utf-8")),
                checksum_sha256="checksum-a2a-contract",
            ...
            assert any(
                "supported public v1 surface" in note.lower()
                for note in agent_card["compatibility_notes"]
            )
            ...
            assert task["status"]["internal_status"] == "queued"
            ...
            assert initial_events[0]["api_version"] == "v1"
            assert initial_events[0]["contract_version"] == "a2a.public.task.event.v1"
            assert initial_events[1]["change"]["artifact"]["id"] == artifact.id

            completed_job = replace(
                job,
                status="completed",
                result_summary="The contract task completed successfully.",
            ...
            completion_artifact = ArtifactRecord(
                id="art_a2a_contract_complete",
                job_id=job.id,
                session_id=session_id,
                source_message_id=None,
                artifact_type="final_text",
                title="Completion note",
                content_text="The contract task completed successfully.",
                file_path=None,
                file_name="completion.txt",
                mime_type="text/plain",
                size_bytes=len("The contract task completed successfully.".encode("utf-8")),
                checksum_sha256="checksum-a2a-contract-complete",
            ...
            assert refreshed_task["summary"] == "The contract task completed successfully."
            assert len(refreshed_task["artifacts"]) == 2
            assert refreshed_task["artifacts"][-1]["is_primary"] is True
            ...
            assert replay_body["task_id"] == task["task_id"]
            ...
            assert [event["sequence"] for event in replay_body["events"]] == [1, 2, 3, 4, 5]
            assert replay_body["events"][2]["change"]["field"] == "status"
            assert replay_body["events"][4]["contract_version"] == "a2a.public.task.event.v1"

            stream_response = client.get(f"/api/v1/a2a/tasks/{task['task_id']}/stream")
            ...
            assert '"next_cursor_sequence": 5' in stream_text
    finally:
        app_main.get_config.cache_clear()
